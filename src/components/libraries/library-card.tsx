...
import { Star, MapPin, Armchair, Wifi } from "lucide-react";
      ...
      >
        ...
        <div className="p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-y-2 border-t border-border pt-4 text-[12.5px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1.5">
              <Wifi className="h-3.5 w-3.5" /> {library.hours}
            </span>
            <span
              className={`flex items-center gap-1.5 font-medium ${library.status === "open" ? "text-accent" : "text-muted-foreground"}`}
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${library.status === "open" ? "bg-accent" : "bg-border-strong"}`}
              />
              {library.status === "open" ? "Open" : "Closed"}
            </span>
          </div>
        </div>
      </TiltCard>
